 ...
 *
 * Solution: O(N) All the 1s to be grouped together would mean that all
...
public class MinimumSwapsToGroupAll1Together {
    public static void main(String[] args) {
        System.out.println(new MinimumSwapsToGroupAll1Together().minSwaps(new int[]{1,0,1,0,1,0,0,1,1,0,1}));
        //
    }
    public int minSwaps(int[] data) {
        // find 1 counts
        var counts = Arrays.stream(data).sum();
        if (counts == 1) {
            return 0;
        }
        // sliding window to find max 1 count.
        int max = 0;
        for (int i=0; i<counts; i++) {
            max += data[i];
        }
        var cur = max;
        for (int i=counts; i < data.length; i++) {
            cur = cur - data[i-counts] + data[i];
            max = Math.max(max, cur);
        }
        // only need to swap 0 at the target window
        return counts - max;
    }
